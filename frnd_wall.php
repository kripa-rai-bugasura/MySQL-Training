<?php
	...
	$res = mysqli_query($conn,"SELECT name FROM tUser WHERE user_id= $frnd_id");
	$user = mysqli_fetch_assoc($res); 
	$walls = mysqli_query($conn, "SELECT * FROM tWall WHERE user_id = $frnd_id " );
?>
<!DOCTYPE html>
<html lang="en">
<head>
					...
					if (mysqli_num_rows($walls) > 0) {
						echo "<ol>";
							while($row = mysqli_fetch_assoc($walls)) {
								...
								echo '<div class="card">';
								echo "<h3>".$row["post"]."</h3>";
								echo "<p><i>".htmlspecialchars($date)."</i></p>";
								echo "</div>";
							}
						echo "</ol>";
					} else {
